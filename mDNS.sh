...
if [[ -z $NAME ]]; then
	echo -e "[${RED}X${NC}] Must setup Name." >&2
	echo "Usage: $0 [-a IP] [-n Name]" >&2
	echo "Example: $0 -a 10.167.214.72 example" >&2
	exit 1
fi
...
if [[ -n $DNS_RESULT ]]; then
	echo -e "[${RED}X${NC}] 網域已經被$DNS_RESULT註冊走了，請換一個。" >&2
	exit 1
fi
